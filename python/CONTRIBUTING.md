...
The source code consists of pure Python 3. To prepare a package for distribution:

To prepare a package for distribution:

1. Make sure all tests pass and that the documentation builds, as described above.

2. Add the description of the new version at the top of the CHANGES.md file.
...
4. If the build succeeds, commit and push your final changes:
```sh
git commit -am "Description of the changes."
git push
```

5. Tag the git commit with the next version and a release candidate number:
```sh
git tag v_._._rc1
git push v_._._rc1
```

6. Check that the package on PyPI works with pip install and check the documentation on readthedocs.com.