...
def build(destination_path: Path | str | None = None) -> Path:
    ...
    if ret_value != 0:
        ...
        print(f'Removing temporary directory {apidoc_path} and any left-behind autogenerated files...')
        shutil.rmtree(apidoc_path, ignore_errors=True)
        for _ in 'modindex.rst', 'genindex.rst':
            (source_path / _).unlink(missing_ok=True)
