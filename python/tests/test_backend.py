...
class BaseTestBackEnd(unittest.TestCase):
    ...
    def test_dtype(self):
        npt.assert_equal(self.BE.hardware_dtype == self.hardware_dtype, True,
    ...
    def test_mat3_eigh(self):
        ...
        npt.assert_array_almost_equal(np.sort(self.BE.asnumpy(self.BE.mat3_eigh(rot_x_d(0.1))[:, 0, 0, 0])),
                                      np.sort(np.linalg.eig(rot_x_d(0.1)[:, :, 0, 0, 0])[0], axis=0), decimal=6,
                                      err_msg='Three small real eigenvalues not detected correctly.')

        npt.assert_array_almost_equal(np.sort(self.BE.asnumpy(self.BE.mat3_eigh(rot_x_d(0.01))[:, 0, 0, 0])),
